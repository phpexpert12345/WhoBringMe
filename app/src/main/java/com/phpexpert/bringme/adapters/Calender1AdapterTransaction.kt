...
class Calender1AdapterTransaction(
        private var context: Context,
        private var commentCalenderView: TransactionFilterLayoutBinding
) :
        RecyclerView.Adapter<Calender1AdapterTransaction.CanceledShipmentViewHolder>() {
    ...
    override fun onBindViewHolder(holder: CanceledShipmentViewHolder, position: Int) {
        ...
        commentCellBinding.root.setOnClickListener {
            if (arrayListDate[position].date != "") {
                ...
                commentCalenderView.startDate.text = Editable.Factory.getInstance().newEditable(changeDateTime("$selectedYearGlobal-${selectedMonthGlobal + 1}-${arrayListDate[position].date}"))
                commentCalenderView.calenderLayout.visibility = View.GONE
